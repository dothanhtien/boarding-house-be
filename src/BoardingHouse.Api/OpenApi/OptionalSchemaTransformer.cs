...
public sealed class OptionalSchemaTransformer : IOpenApiSchemaTransformer
{
    public async Task TransformAsync(OpenApiSchema schema, OpenApiSchemaTransformerContext context, CancellationToken cancellationToken)
    {
        ...
        // Collections (e.g. Optional<List<UpdateRoomAmenityRequest>>): the item schema is generated by the
        // regular pipeline, so registered transformers (including this one) still apply to its properties.
        if (TryGetCollectionElementType(innerType, out var elementType))
        {
            schema.Type = JsonSchemaType.Array | nullFlag;
            schema.Items = await context.GetOrCreateSchemaAsync(elementType, cancellationToken: cancellationToken);
            return;
        }
    ...
    private static bool TryGetCollectionElementType(Type type, out Type elementType)
    {
        ...
        // string is IEnumerable<char> but is rendered as a JSON string, handled above.
